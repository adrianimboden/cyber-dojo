...
size_t global_amount_of_calls = 0;

size_t very_expensive_calculation()
{
    //track amount of expensive calculations
    ++global_amount_of_calls;
    return 5; //expensive calculation
};

TEST(Memoizer, memoizing_with_free_function_works)
{
    size_t previous = global_amount_of_calls;

    // create memoizer
    auto memoizer = Memoizer<int>{very_expensive_calculation};

    // memoizer calculates nothing in advance
    ASSERT_THAT(global_amount_of_calls, Eq(previous));
    ...
    // memoizer needs to calculate only once
    ASSERT_THAT(global_amount_of_calls, Eq(previous + 1));
}
